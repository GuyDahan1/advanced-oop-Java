...
public class AnimalFactory {
    ...
    public Animal getAnimal(String animalName, double speed , Point staarPoint, CompetitionPanel panel, String ImgChoice){
        if(ImgChoice.toLowerCase().contains(Alligator.toLowerCase()))
            return new Alligator(animalName,speed,staarPoint,panel,ImgChoice);
        else if(ImgChoice.toLowerCase().contains(Cat.toLowerCase()))
            return new Cat(animalName,speed,staarPoint,panel,ImgChoice);
        else if(ImgChoice.toLowerCase().contains(Dog.toLowerCase()))
            return new Dog(animalName,speed,staarPoint,panel,ImgChoice);
        else if(ImgChoice.toLowerCase().contains(Dolphin.toLowerCase()))
            return new Dolphin(animalName,speed,staarPoint,panel,ImgChoice);
        else if(ImgChoice.toLowerCase().contains(Eagle.toLowerCase()))
            return new Eagle(animalName,speed,staarPoint,panel,ImgChoice);
        else if(ImgChoice.toLowerCase().contains(Pigeon.toLowerCase()))
            return new Pigeon(animalName,speed,staarPoint,panel,ImgChoice);
        else if(ImgChoice.toLowerCase().contains(Snake.toLowerCase()))
            return new Snake(animalName,speed,staarPoint,panel,ImgChoice);
        else if(ImgChoice.toLowerCase().contains(Whale.toLowerCase()))
            return new Whale(animalName,speed,staarPoint,panel,ImgChoice);
        return null;
    }
}
